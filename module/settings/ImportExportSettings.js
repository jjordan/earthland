...
export default class ImportExportSettings extends FormApplication {
  ...
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      ...
      template: 'systems/earthland/templates/import-export-settings.html',
      title: localizer('ImportExportSettings'),
      classes: ['earthland', 'import-export-settings'],
      width: 'auto',
      height: 'auto',
      top: 200,
      left: 400,
  ...
  getData() {
    return game.settings.get('earthland', 'importedSettings')
  }
  ...
  async _exportSettings(event) {
    ...
    const { current, custom } = await game.settings.get('earthland', 'themes')

    const settings = {
      actorTypes: game.settings.get('earthland', 'actorTypes'),
      earthlandVersion: game.system.data.version,
  ...
  async _importSettings(event) {
    ...
    if (file) {
      ...
      fileReader.onload = async () => {
        ...
        if (!data?.earthlandVersion && !data?.actorTypes) {
          ui.notifications.error(localizer('CantReadImportFile'))
          return
        }

        if (game.system.data.version !== data?.earthlandVersion) {
        ...
        if (confirmed) {
          await game.settings.set('earthland', 'importedSettings', { currentSetting: file.name })
          await game.settings.set('earthland', 'actorTypes', data.actorTypes)

          const themeSettings = await game.settings.get('earthland', 'themes')
          ...
          await game.settings.set('earthland', 'themes', themeSettings)
  ...
  async _resetSettings (event) {
    ...
    if (confirmed) {
      await game.settings.set('earthland', 'importedSettings', { currentSetting: localizer('Default') })
      await game.settings.set('earthland', 'actorTypes', defaultActorTypes)
