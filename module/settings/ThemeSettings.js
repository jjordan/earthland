...
export default class ThemeSettings extends FormApplication {
  ...
  async _updateObject(event, formData) {
    ...
    const currentThemes = game.settings.get('earthland', 'themes') ?? {}
    ...
    await game.settings.set('earthland', 'themes', mergeObject(currentThemes, expandedFormData.themes))
  ...
  async _changeImage (event) {
    ...
    const source = game.settings.get('earthland', 'themes')
    ...
    const imagePicker = await new FilePicker({
      ...
      async callback (newImage) {
        ...
        await game.settings.set('earthland', 'themes', source)
